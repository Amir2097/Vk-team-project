...
class ExtractingUserData:
    ...
    def user_search(self, count, age_from, age_to, sex, city=1, country=1):
        ...
        city - идентификаттор города, берется у пользователя который ведет диалог с ботом (по умолчанию Москва);
        country - идентификатор страны, берется у пользователя который ведет диалог с ботом (по умолчанию Россия).
